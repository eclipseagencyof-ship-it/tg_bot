...
async def objection_rules(cq: types.CallbackQuery):
    # --- мгновенно закрываем callback, чтобы Telegram не ругался ---
    asyncio.create_task(cq.answer())

    # --- Отправляем первую картинку и текст ---
    photo1_path = IMAGES_DIR / "rules1.jpg"
    text1 = (
        "<b>📋 Ниже будет список запретов непосредственно от OnlyFans:</b>\n\n"
        "🚫 Контент с третьими лицами без согласия и модельного релиза\n"
        "🚫 Несовершеннолетние (даже ролевые игры)\n"
        "🚫 Оружие, наркотики\n"
        "🚫 Самоповреждения, инцест, зоофилия и т.д.\n"
        "🚫 Домашние животные в кадре при откровённом контенте — нарушение\n"
    )

    if photo1_path.exists():
        await bot.send_photo(cq.from_user.id, photo=open(photo1_path, "rb"), caption=text1, parse_mode="HTML")
    else:
        await bot.send_message(cq.from_user.id, "⚠️ Картинка с правилами не найдена.", parse_mode="HTML")

    # --- задержка не обязательна, но можно оставить ---
    await asyncio.sleep(1.5)

    # --- второй блок + кнопка ---
    text2 = (
        "🚫 Насилие, изнасилование, отсутствие согласия, гипноз, опьянение\n"
        "🚫 Некрофилия, экскременты, моча, рвота\n"
        "🚫 Эскорт, проституция, реклама без разрешения\n"
        "🚫 Контент +18 в публичных местах\n"
        "🚫 Распространение личных данных\n"
        "🚫 Шантаж, угрозы, копирование чужого контента\n"
        "🚫 Накрутки, спам, кодовые слова (meet, paypal, young...)\n\n"
        "<b>⚠️ Соблюдение этих правил — гарантия безопасности и стабильной работы аккаунта.</b>"
    )
