...
async def objection_rules(cq: types.CallbackQuery):
    # корректно отвечаем callback, чтобы Telegram не завис
    await cq.answer()

    # 🖼️ Отправка картинки + текста
    photo1_path = IMAGES_DIR / "rules1.jpg"
    try:
        photo1 = types.InputFile(photo1_path)
        text1 = (
        ...
        await bot.send_photo(cq.from_user.id, photo=photo1, caption=text1, parse_mode="HTML")
    except FileNotFoundError:
        await bot.send_message(cq.from_user.id, "⚠️ Изображение с правилами не найдено.")
    ...
    text2 = (
        ...
        "садомазохистское насилие или жёсткий бондаж, экстремальный фистинг или калечащие операции на половых органах.\n"
        "🚫 Некрофилия\n"
        "🚫 Контент с мочой, рвотой, экскрементами\n"
        "🚫 Эскорт, проституция\n"
        "🚫 Оскорбления, унижения, дискриминация\n"
        "🚫 Личные данные, документы, телефоны, логины и пр.\n"
        "🚫 Контент +18 в публичных местах\n"
        "🚫 Шантаж (blackmail)\n"
        "🚫 Продажа товаров/услуг без разрешения OnlyFans\n"
        "🚫 Копирование чужого контента\n"
        "🚫 Спам, накрутки, искусственная активность\n"
        "🚫 Использование записанных видео как «прямого эфира»\n"
        "🚫 Кодовые слова для обхода модерации (meet, paypal, young и т.д.)\n\n"
