...
# 🟢 Реакция на кнопку "💡 Зачем нужны разные рассылки?"
@dp.callback_query_handler(lambda c: c.data == "diff_mailings")
async def diff_mailings_info(cq: types.CallbackQuery):
    await cq.answer()

    text = (
        "Разные рассылки нужны, чтобы повышать конверсию и не терять клиентов 👇\n\n"
        "💎 *VIP* — ждут особого отношения. Если шлёшь им то же, что и всем — теряешь лояльность.\n\n"
        "🟢 *Онлайн* — быстро реагируют. Именно отсюда идут импульсивные покупки ⚡️\n\n"
        "📣 *Массовая* — работает на прогрев, напоминание и удержание интереса.\n\n"
        "📊 Комбинируй все три — и у тебя будет стабильный поток продаж 🔥"
    )
    await bot.send_message(cq.from_user.id, text, parse_mode="Markdown")
