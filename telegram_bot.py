...
async def objection_rules(cq: types.CallbackQuery):
    ...
    text2 = (
        ...
        "🚫 Распространение личных данных, частной или конфиденциальной информации. Например, номера телефонов, информация о конкретном местоположении(просто сказать из какой вы страны не считается,это ок), документы, адреса электронной почты, учетные данные для входа в OnlyFans, финансовую информацию(сюда входят любые попытки провести оплату вне онлика)\n"
    ...
    kb_next = InlineKeyboardMarkup().add(
        InlineKeyboardButton("⭐ А что насчет запретов агентства?", callback_data="rules_agency")
    )
