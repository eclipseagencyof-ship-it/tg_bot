...
async def objection_rules(cq: types.CallbackQuery):
    # Сразу отвечаем callback, чтобы не поймать InvalidQueryID
    asyncio.create_task(safe_answer(cq))  # безопасный ответ (асинхронно, не блокирует)
    # или если safe_answer нет — просто так:
    # asyncio.create_task(cq.answer())

    # 🖼️ Отправка картинки + текста
    photo1 = IMAGES_DIR / "rules1.jpg"
    text1 = (
        ...
        "🚫 Зоофилия. Всех своих котиков и собачек лучше убрать. "
        "Были случаи, когда кошечка модели случайно попала в кадр при съемке контента, "
        "а за это страница получила предупреждение\n"
    )
    await bot.send_photo(cq.from_user.id, photo1, caption=text1, parse_mode="HTML")

    # ⏳ Небольшая задержка (для плавности)
    await asyncio.sleep(2)

    # 🧾 Следующий блок текста + кнопка
    text2 = (
        "🚫 Насилие, изнасилование, отсутствие согласия, гипноз, опьянение, сексуальное нападение, пытки, "
        "садомазохистское насилие или жесткий бондаж, экстремальный фистинг или калечащие операции на половых органах. "
        "Тут для себя понимаем, что с БДСМ контентом и играми в жестких доминаторов лучше быть аккуратнее\n\n"
        "🚫 Некрофилия\n"
        "🚫 Контент с мочой, рвотой, экскрементами\n"
        "🚫 Эскорт-услуги, секс-торговлю или проституцию\n"
        "🚫 Контент, направленный на очернение, унижение, угрозы или возбуждение ненависти, страха или насилия "
        "в отношении любой группы людей или одного человека по любой причине (раса, пол, внешность и т.д.)\n"
        "🚫 Распространение личных данных, частной или конфиденциальной информации "
        "(номера телефонов, документы, email, логины, финансовые данные и т.д.)\n"
        "🚫 Контент +18, если он был записан или транслируется из публичного места, где прохожие могут увидеть действия\n"
        "🚫 Шантаж, угрозы (blackmail)\n"
        "🚫 Продажа товаров, реклама, вакансии — без разрешения администрации сайта\n"
        "🚫 Нарушение прав создателей — запись, копирование, публикация чужого контента\n"
        "🚫 Спам, искусственное повышение активности\n"
        "🚫 Использование заранее записанных видео во время «прямого эфира»\n"
        "🚫 Использование кодовых слов для обхода модерации (meet, paypal, young и т.д.)\n\n"
...
# --- 2️⃣ Кнопка: "⭐ А что насчет запретов агентства?" ---
...
async def rules_agency(cq: types.CallbackQuery):
    ...
    kb_checklist = InlineKeyboardMarkup().add(
        InlineKeyboardButton("📋 Чек-лист", callback_data="show_checklist")
    )
...
async def show_checklist(cq: types.CallbackQuery):
    ...
    await bot.send_message(
        cq.from_user.id,
        "✅ Отлично! Ты завершил вводную часть. Теперь можно переходить к обучению продаж 💬",
        parse_mode="HTML",
    )
